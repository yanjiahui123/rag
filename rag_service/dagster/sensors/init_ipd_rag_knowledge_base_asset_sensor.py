...
from rag_service.dagster.assets.init_ipd_rag_knowledge_base_asset import (
    init_ipd_rag_knowledge_base_asset,
)
...
from rag_service.dagster.jobs.init_ipd_rag_knowledge_base_asset_job import init_ipd_rag_knowledge_base_asset_job
...
@sensor(job=init_ipd_rag_knowledge_base_asset_job, default_status=DefaultSensorStatus.RUNNING)
def init_ipd_rag_knowledge_base_asset_sensor():
    with Session(engine) as session:
        pending_jobs: List[AutoJobInstances] = get_pending_jobs_by_type(VectorizationJobType.INIT_IN_IPD_RAG, session)
        ...
        return SensorResult(
            run_requests=[
                RunRequest(
                    ...
                    run_config={
                        "ops": {
                            init_ipd_rag_knowledge_base_asset.node_def.name: {
                                "ops": {

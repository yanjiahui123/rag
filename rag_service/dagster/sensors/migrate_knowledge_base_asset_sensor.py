...
from rag_service.dagster.assets.migrate_knowledge_base_asset_to_ipd_rag import (
    migrate_knowledge_base_asset_to_ipd_rag,
)
...
from rag_service.dagster.jobs.migrate_knowledge_base_asset_to_ipd_rag_job import (
    migrate_knowledge_base_asset_to_ipd_rag_job,
)
...
@sensor(job=migrate_knowledge_base_asset_to_ipd_rag_job, default_status=DefaultSensorStatus.RUNNING)
def migrate_knowledge_base_asset_to_ipd_rag_sensor():
    with Session(engine) as session:
        pending_jobs: List[AutoJobInstances] = get_pending_jobs_by_type(
            VectorizationJobType.MIGRATE_TO_IPD_RAG, session
        )
        ...
        return SensorResult(
            run_requests=[
                RunRequest(
                    ...
                    run_config={
                        "ops": {
                            migrate_knowledge_base_asset_to_ipd_rag.node_def.name: {
                                "ops": {
